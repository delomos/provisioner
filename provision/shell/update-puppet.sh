...
if [[ ! -f /.provision-stuff/update-puppet ]]; then
    if [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
        echo "Downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        wget --quiet --tries=5 --timeout=10 -O "/.provision-stuff/puppetlabs-release-${CODENAME}.deb" "http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"
        echo "Finished downloading http://apt.puppetlabs.com/puppetlabs-release-${CODENAME}.deb"

        dpkg -i "/.provision-stuff/puppetlabs-release-${CODENAME}.deb" >/dev/null
        ...
        touch /.provision-stuff/update-puppet
        echo "Created empty file /.provision-stuff/update-puppet"
    elif [ "${OS}" == 'centos' ]; then
