...
if [ "${FOUND_GIT}" -ne '0' ] && [ ! -f /.provision-stuff/librarian-puppet-installed ]; then
...
if [ "${OS}" == 'debian' ] || [ "${OS}" == 'ubuntu' ]; then
    if [[ ! -f /.provision-stuff/librarian-base-packages ]]; then
        ...
        touch /.provision-stuff/librarian-base-packages
    fi
fi

if [ "${OS}" == 'ubuntu' ]; then
    if [[ ! -f /.provision-stuff/librarian-libgemplugin-ruby ]]; then
        ...
        touch /.provision-stuff/librarian-libgemplugin-ruby
    fi

    if [ "${CODENAME}" == 'lucid' ] && [ ! -f /.provision-stuff/librarian-rubygems-update ]; then
        ...
        touch /.provision-stuff/librarian-rubygems-update
    fi
fi

if [[ ! -f /.provision-stuff/librarian-puppet-installed ]]; then
    ...
    touch /.provision-stuff/librarian-puppet-installed
else
...
git clone https://github.com/provision/puppetlabs-git.git /etc/puppet/modules/git
