...
exec { 'dotfiles':
  ...
  command => "cp -r /vagrant/provision/files/dot/.[a-zA-Z0-9]* /home/${::ssh_username}/ \
              && chown -R ${::ssh_username} /home/${::ssh_username}/.[a-zA-Z0-9]* \
              && cp -r /vagrant/provision/files/dot/.[a-zA-Z0-9]* /root/",
  onlyif  => 'test -d /vagrant/provision/files/dot',
  returns => [0, 1],
...
if $yaml_values == undef {
  $yaml_values = loadyaml('/vagrant/provision/config.yaml')
}
